...
def get_input_symbols(spec_lines):
    ...
    for i,l in enumerate(spec_lines):
        if l.strip().startswith('i') and not start:
            start = i
        if l.strip() == 'c':
            end = i
...
def get_control_inputs(spec_lines):
    control_inputs = set()
    input_symbols = get_input_symbols(spec_lines)
    for s in input_symbols:
        ...
        if tokens[1].startswith('c'):
            ...
            input_literal = int(spec_lines[1+input_index])
...
def main(original_lines, synthesized_lines):
    ...
    assert orig_non_control_definitions.issubset(synth_non_control_definitions), \
        'some gates are missing in synthesized file'
